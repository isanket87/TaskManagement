...
// ── Status Cell ─────────────────────────────────────────────────────
const StatusCell = ({ tasks, memberId, status, focusedMemberId, onDrop, projectId }) => {
            ...
            {tasks.map((task) => (
                <div
                    ...
                    className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                        ...
                        cursor-grab active:cursor-grabbing select-none leading-snug
                        hover:border-orange-300 hover:bg-orange-50 dark:hover:border-orange-700 dark:hover:bg-orange-900/20
                        transition-all hover:-translate-y-px hover:shadow-sm"
                >
                    {task.title}
                </div>
            ))}
            ...
            <div className="overflow-x-auto pb-4 shrink-0">
                ...
                <div className="grid gap-3 mb-3" style={{ gridTemplateColumns: '160px repeat(4, 1fr)' }}>
                    ...
                    {COLUMNS.map((col) => (
                        <div
                            key={col.id}
                            className="flex flex-col items-center gap-1 py-2 px-3 text-center
                ...
                {members.map((member, rowIdx) => {
                    ...
                    return (
                        ...
                        >
                            {/* Member cell */}
                            <button
                                onClick={() => toggleFocus(member.user.id)}
                                className={cn(
