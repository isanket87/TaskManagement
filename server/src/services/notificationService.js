 ...
 */
const createNotification = async ({ userId, type, message, link, taskId, projectId, projectName, taskTitle }) => {
    try {
 ...
 */
const notifyTaskAssigned = async ({ task, assignee, assignedBy, project }) => {
 ...
 */
const notifyTaskOverdue = async ({ task, assignee, project }) => {
    await createNotification({
        userId: assignee.id,
        type: 'task_overdue',
 ...
 */
const notifyTaskDueSoon = async ({ task, assignee, project }) => {
    await createNotification({
        userId: assignee.id,
        type: 'task_due_soon',
 ...
 */
const notifyCommentMention = async ({ task, project, mention, author, commentText }) => {
    await createNotification({
        userId: mention.id,
 ...
 * Daily digest
 */
const sendDailyDigestForUser = async ({ user, overdueTasks, dueTodayTasks, dueThisWeekCount }) => {
